...
- 接受系统提示词和用户提示词文本输入
- 接受最多八个图片输入（可选）
- 支持模型选择（"gemini-3.1-flash-image", "gemini-3-pro-image", "gemini-2.5-flash-image"）
- 支持尺寸选择（1024x1024, 2048x2048, 4096x4096）
- 支持比例选择（1:1, 4:3, 3:4, 16:9, 9:16, 3:2, 2:3）
- 将数据打包后调用后端API接口
- 输出API调用的返回结果

### 3. GPT Image节点（API365_GPTImage）
- 接受用户提示词文本输入
- 支持模型选择 ("gpt-image-1.5", "gpt-image-2")
- 支持尺寸选择（1024x1024, 1024x1536, 1536x1024）
- 将数据打包后调用后端API接口
- 输出API调用的返回结果
